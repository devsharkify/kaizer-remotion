...
const VideoClip: React.FC<{ src: string; startSec: number; heightPx: number }> = ({ src, startSec, heightPx }) => {
  ...
  return (
    <div style={{ position: "absolute", top: 0, left: 0, width, height: heightPx, overflow: "hidden", background: "#111" }}>
      <OffthreadVideo src={src} startFrom={Math.round(startSec * 30)} style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "100%", minHeight: "100%", objectFit: "cover" }} />
    </div>
  );
};
...
const AnimatedStrip: React.FC<{ line1: string; line2: string; topPx: number; heightPx: number }> = ({ line1, line2, topPx, heightPx }) => {
  ...
  return (
    <div style={{ position: "absolute", top: topPx, left: 0, width, height: heightPx, transform: `translateY(${translateY}px)`, zIndex: 20 }}>
      ...
      <div style={{ position: "absolute", top: "28%", width: "100%", textAlign: "center", fontFamily: "sans-serif", fontWeight: 700, fontSize: 72, color: "white", textShadow: "0 2px 16px rgba(0,0,0,0.9)", opacity: l1Opacity, padding: "0 40px" }}>{line1}</div>
      <div style={{ position: "absolute", top: "58%", width: "100%", textAlign: "center", fontFamily: "sans-serif", fontWeight: 900, fontSize: 88, color: "#FFD700", textShadow: "0 3px 20px rgba(0,0,0,1)", opacity: l2Opacity, transform: `scale(${interpolate(l2Spring, [0, 1], [0.6, 1])})`, transformOrigin: "center center", padding: "0 40px" }}>{line2}</div>
    </div>
  );
};
...
const KaizerShort: React.FC<KaizerClipProps> = ({ videoSrc, startSec, endSec, line1, line2, words, ratio, bottomImageSrc }) => {
  ...
  if (ratio === "9:16") {
    videoHeight = 1020; stripTop = 880; stripHeight = 360; imageTop = 1160; captionBottom = 420;
  } else if (ratio === "9:10") {
    videoHeight = 640; stripTop = 540; stripHeight = 240; imageTop = 700; captionBottom = 300;
  } else {
    videoHeight = height; stripTop = height - 280; stripHeight = 240; imageTop = height; captionBottom = 300;
  }
  return (
